...
router.put("/posts/:id", (req, res, next) => {
  ...
  if (typeof(req.body.author) != "undefined") { post['author'] = req.body.author}
  if (typeof(req.body.content) != "undefined") { post['content'] = req.body.content}

  dbConnect.collection("posts").updateOne( { _id: ObjectId(req.params.id) }, { $set: post}, function (err, result) {
    if (err) {
      ...
      dbConnect
        ...
        .limit(1)
        .toArray( (err, result) => { res.status(200).json(result); });
    }
  });
});
